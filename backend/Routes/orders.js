...
router.post("/orders", async (req, res) => {
  try {
    const {
      vehicle_id, user_id, driver_id, original_price, rental_price,
...
const discountAmount = parseFloat((original_price * discountPercent / 100).toFixed(2));
const discountedPrice = parseFloat((original_price - discountAmount).toFixed(2));
  ...
  INSERT INTO orders (
    ...
    loyalty_discount, loyalty_saved, transaction_id,
    created_at
  ) VALUES (
    $1, $2, $3,
    $4, $5,
...
`, [
  ...
  discountPercent,
  discountAmount,
  null // or transaction ID if payment is already done
]);
    ...
    await updateLoyaltyPoints(user_id);
